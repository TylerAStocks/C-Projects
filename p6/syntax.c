...
/** Implementation of && for instances of InfixExpr that do logical AND
    comparison. */
static Value *evalAnd( Expr *expr, Environment *env )
{
  ...
  Value *v1 = this->left->eval( this->left, env );
  if( v1->equals(v1, makeInteger(0) ) ) {
    Value *v2 = this->right->eval( this->right, env ); 
    if ( v2->equals(v2, makeInteger(0) ) ) {
      v1->destroy( v1 );
      v2->destroy( v2 );
      return makeInteger(false);
    } else {
...
{
  // Use the convenience function to build a InfixExpr for the And test.
  return buildInfixExpr( left, right, evalAnd );
}


/** Implementation of || for instances of InfixExpr that do logical OR
    comparison. */
...
{
  // Use the convenience function to build a InfixExpr for the Or test.
